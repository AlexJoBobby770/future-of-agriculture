...
def get_weather_impact():
    ...
    if not os.path.exists(path):
        return 0.0, "☁️ Weather Station Offline"

    with open(path, "r") as f:
        data = json.load(f)
    
    # Open-Meteo usually returns 'rain' in mm
    ...
    if current_rain > 5.0:
        ...
        status = "🌧️ Light Rain: Soil Hydration Optimal"
    else:
        penalty = 0.0
        status = "☀️ Clear Skies: No Leaching Risk"
